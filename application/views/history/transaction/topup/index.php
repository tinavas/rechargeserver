<script type="text/javascript">
    ...
    function search_topup(searchInfo) {
        var startDate = $("#start_date").val();
        var endDate = $("#end_date").val();
        angular.element($("#search_submit_btn")).scope().getTopupTransactionList(startDate, endDate, '<?php echo $user_id; ?>');
    }
</script>
...
<div class="mypage" ng-controller="transctionController" ng-init="setTransactionStatusList('<?php echo htmlspecialchars(json_encode($transction_status_list)) ?>')">
    <ul class="list-unstyled paymentHistorySearch">
        ...
        <li><input id="search_submit_btn" type="submit" size="18" value="Search" onclick="search_topup(angular.element(this).scope().searchInfo)" class="button-custom"></li>
    </ul>
    <table class="table10"> 
        <thead>
            <tr>
    ...
    <div class="other-controller">
        <div class="text-center">
            <dir-pagination-controls boundary-links="true" on-page-change="getTopupByPagination(newPageNumber,'<?php echo $user_id; ?>')" template-url="<?php echo base_url(); ?>history/pagination_tmpl_load"></dir-pagination-controls>
        </div>
    </div>
</div>
<div class="row"></div>
